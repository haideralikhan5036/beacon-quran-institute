        ...
        <Reveal delay={0.5} direction="up" distance={20}>
          <div className="mt-16 text-center card-shine border border-[#757454]/40 p-8 sm:p-10 rounded-[2.5rem] max-w-3xl mx-auto text-amber-50 shadow-2xl">
             ...
             <p className="text-amber-50 font-medium text-base sm:text-lg leading-relaxed">
               {isAr ? (
                 <>هل تحتاج إلى خطة مرنة تناسب الإخوة والأخوات أو حلقات دراسية مكثفة؟ <span className="text-amber-200 font-bold underline underline-offset-4 decoration-[#757454] hover:text-white transition-colors cursor-pointer" onClick={() => handlePlanSelect('Flexible Plan')}>تواصل مع مكتب التسجيل والقبول</span></>
               ) : (
                 <>Need a custom plan for family siblings or specialized intensive courses? <span className="text-amber-200 font-bold underline underline-offset-4 decoration-[#757454] hover:text-white transition-colors cursor-pointer" onClick={() => handlePlanSelect('Flexible Plan')}>Contact our registrar</span></>
               )}
             </p>
          </div>
        </Reveal>
